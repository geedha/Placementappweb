...
@WebServlet("/getplacement")
public class Getplaceement extends HttpServlet{
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try{  
            ...
            while(rs.next()) 
            listPlacement.add(new Placement(rs.getString(1),rs.getString(2),rs.getString(3),rs.getFloat(4),rs.getString(5),rs.getInt(6)));
            System.out.println(rs.getString(1));
            con.close();  
            }catch(Exception e){ System.out.println(e);} 
            
           JsonArray json = new Gson().toJsonTree(listPlacement).getAsJsonArray();   
